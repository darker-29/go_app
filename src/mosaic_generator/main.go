			...
			for x := x1; x < x2; x = x + tileSize {
				...
				if err == nil {
					...
					if err == nil {
						...
						fmt.Println("error in decoding nearest:", err)
					}
				} else {
...
func combine(r image.Rectangle, c1, c2, c3, c4 <-chan image.Image) <-chan string {
	...
	go func() {
		...
		for {
			...
			case s2, ok2 = <-c2:
				go copy(img, s2.Bounds(), s2, image.Point{r.Min.X / 2, r.Min.Y})
			case s3, ok3 = <-c3:
				go copy(img, s3.Bounds(), s3, image.Point{r.Min.X, r.Min.Y / 2})
			case s4, ok4 = <-c4:
				go copy(img, s4.Bounds(), s4, image.Point{r.Min.X / 2, r.Min.Y / 2})
			}
